...
{
    FILE *fichier = fopen("../fichiers/tableauTest/test.txt", "r");
    ...
    {
        printf("\ndatePlusTot de %d => %d\n",i+1,DateAuPlusTot(i+1,TabFileP,tabDurees,tabSommets));
    }
    
    printf("\n\n ----------------Date au plus tard------------\n\n");
    for (int i = 0; i < nbrSommets(); i++)
    {
        printf("\ndatePlusTard de %d => %d\n",i+1,DateAuPlusTard(i+1,TabFileP,TabFileS,tabDurees,tabSommets));
    }
    
    printf("\n\n ----------------Marge  Totale------------\n\n");
    for (int i = 0; i < nbrSommets(); i++)
    {
        printf("\nmargeTotale de %d => %d\n",i+1,margeTotale(i+1,TabFileP,TabFileS,tabDurees,tabSommets));
    }
